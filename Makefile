...
check-git:
	...
	@if ! git remote -v | grep "upstream[[:space:]]git@github.com:ethereum/eth-rlp.git (push)\|upstream[[:space:]]https://github.com/ethereum/eth-rlp (push)"; then \
		echo "Error: You must have a remote named 'upstream' that points to 'eth-rlp'"; \
		exit 1; \
	fi
